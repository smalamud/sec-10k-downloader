...
\begin{frame}{What Changed?}
  \begin{center}
    \begin{tikzpicture}[
      box/.style={rounded corners=8pt, minimum width=3.4cm, minimum height=1.6cm,
      ...
      \node[box, fill=blue!10, right=1.2cm of old] (ml) {
        \textbf{Machine Learning}\\[4pt]
        Learns patterns\\from data\\[2pt]
        {\footnotesize Fraud detection}
      };
      \node[box, fill=green!10, right=1.2cm of ml] (llm) {
        \textbf{LLMs (2023--)}\\[4pt]
...
\begin{frame}{What is RAG?}
  ...
  \begin{center}
    \begin{tikzpicture}[
      stp/.style={rounded corners=8pt, minimum width=2.6cm, minimum height=1.8cm,
                   text width=2.2cm, align=center, font=\small, inner sep=5pt},
      ...
      \node[stp, fill=amber!15, right=0.7cm of q] (search) {
        \faSearch\\[4pt]
        \textbf{Search}\\[2pt]
        {\footnotesize Find relevant paragraphs}
      };

      \node[stp, fill=green!12, right=0.7cm of search] (read) {
        \faFile\\[4pt]
        \textbf{Read}\\[2pt]
        {\footnotesize AI reads top 5 results}
      };

      \node[stp, fill=purple!12, right=0.7cm of read] (ans) {
        \faRobot\\[4pt]
        \textbf{Answer}\\[2pt]
...
\begin{frame}{How RAG Works Under the Hood}
  \begin{center}
    \begin{tikzpicture}[scale=0.85, every node/.style={transform shape},
      box/.style={rounded corners=6pt, minimum height=1.3cm, text width=2.8cm,
                  align=center, font=\small, inner sep=5pt},
      db/.style={rounded corners=6pt, draw=accent, fill=blue!8,
                 minimum width=2.2cm, minimum height=1.5cm, font=\small, align=center, inner sep=5pt},
      ...
      % Chunking
      \node[box, fill=amber!15] (chunk) at (3.8,2) {
        \faCut~\textbf{Chunk}\\
      ...
      \node[box, fill=green!10] (retrieve) at (3.8,-0.5) {
        \faSearch~\textbf{Retrieve}\\
        {\footnotesize Top 5 matches}
      };

      \node[box, fill=purple!10] (llm) at (7.6,-0.5) {
        \faRobot~\textbf{LLM}\\
        {\footnotesize Synthesise answer}
      };

      \node[box, fill=accent2!10] (answer) at (11.4,-0.5) {
...
\begin{frame}[shrink=10]{Embeddings: How Machines ``Understand'' Text}
  \begin{center}
...
\begin{frame}{From Chatbot to Agent}
  \begin{center}
    \begin{tikzpicture}[
      box/.style={rounded corners=8pt, minimum width=5cm, minimum height=3cm,
                  text width=4.6cm, align=left, font=\small, inner sep=8pt}
    ]
      ...
      \node[box, fill=accent!8, right=0.8cm of chat] (agent) {
...
\begin{frame}[shrink=5]{The Agentic Loop}
  \begin{center}
    \begin{tikzpicture}[
      stp/.style={rounded corners=8pt, minimum width=2cm, minimum height=1.3cm,
      ...
      \node[stp, fill=amber!20] (plan) at (2.6,0) {\faClipboardList\\Plan};
      \node[stp, fill=green!15] (act) at (5.2,0) {\faWrench\\Act};
      \node[stp, fill=purple!15] (observe) at (7.8,0) {\faEye\\Observe};
      \node[stp, fill=red!10] (reflect) at (5.2,-2.5) {\faBrain\\Reflect};
      \node[stp, fill=accent2!15] (answer) at (10.4,0) {\faCheckCircle\\Answer};
      ...
      % Loop back
      \draw[arr, accent4] (observe.south) -- ++(0,-0.6) -| (reflect.east);
...
\begin{frame}[shrink=10]{Agent Tools = Analyst Toolkit}
  \begin{center}
    \begin{tikzpicture}[
...
\begin{frame}{Our Dataset}
  \begin{center}
    \begin{tikzpicture}
      \node[rounded corners=10pt, fill=lightbg, text width=10cm, align=center,
            inner sep=12pt, font=\large] {
        \textbf{458 SEC 10-K Filings}\\[6pt]
        \normalsize
        100 S\&P 100 companies $\times$ up to 5 years\\[10pt]
        \begin{tikzpicture}
          \foreach \sector/\col/\x in {
            Tech/accent/0, Banking/accent3/1.7, Health/accent2/3.4,
            Energy/accent4/5.1, Consumer/purple/6.8, Industrial/gray/8.5} {
            \node[rounded corners=4pt, fill=\col!15, minimum width=1.5cm,
                  minimum height=0.6cm, font=\scriptsize\bfseries] at (\x,0) {\sector};
          }
        \end{tikzpicture}
      };
    \end{tikzpicture}
...
\begin{frame}{The Pipeline We Built}
  \begin{center}
    \begin{tikzpicture}[
      stp/.style={rounded corners=6pt, minimum height=1.2cm, text width=2.2cm,
      ...
      \node[stp, fill=amber!15] (clean) at (2.7,0) {\textbf{Clean Text}\\Strip formatting};
      \node[stp, fill=blue!10] (chunk) at (5.4,0) {\textbf{Chunks}\\500-word windows};
      \node[stp, fill=green!10] (embed) at (8.1,0) {\textbf{Vectors}\\Embed text};
      \node[stp, fill=purple!10] (index) at (10.8,0) {\textbf{Index}\\FAISS search};
...
\begin{frame}{The Upgrade Path}
  \begin{center}
    \begin{tikzpicture}[
      level/.style={rounded corners=6pt, minimum width=10cm, minimum height=1.1cm,
                    text width=9.5cm, align=left, font=\small, inner sep=6pt},
      ...
      \draw[decorate, decoration={brace, amplitude=8pt, mirror}, accent]
        (5.8, 4.3) -- (5.8, 2.4) node[midway, right=10pt, font=\footnotesize, text=accent] {Today's demo};
      \draw[decorate, decoration={brace, amplitude=8pt, mirror}, accent2]
        (5.8, 1.7) -- (5.8, -0.2) node[midway, right=10pt, font=\footnotesize, text=accent2] {With OpenAI API};
    \end{tikzpicture}
  \end{center}
\end{frame}

\begin{frame}[shrink=10]{TF-IDF vs OpenAI Embeddings}
  \begin{center}
...
\begin{frame}{Use Cases Across the Bank}
  \begin{center}
    \begin{tikzpicture}[
      card/.style={rounded corners=6pt, minimum width=4.2cm, minimum height=1.8cm,
                   text width=3.8cm, align=left, font=\small, inner sep=6pt}
    ]
      \node[card, fill=blue!8] (risk) at (0,2) {
        \textbf{\color{accent}\faShield*~Risk Mgmt}\\[3pt]
        {\footnotesize ``Flag material litigation across 50 counterparty filings''}
      };
      \node[card, fill=green!8] (comp) at (4.8,2) {
        \textbf{\color{accent2}\faBalanceScale~Compliance}\\[3pt]
        {\footnotesize ``Do portfolio companies disclose TCFD climate risks?''}
      };
      ...
      \node[card, fill=purple!8] (ma) at (4.8,-0.5) {
        \textbf{\color{purple}\faHandshake~M\&A Due Diligence}\\[3pt]
        {\footnotesize ``Review target's risk factors and contingent liabilities''}
      };
      \node[card, fill=red!6] (audit) at (2.4,-2.9) {
        \textbf{\color{accent4}\faClipboardCheck~Audit}\\[3pt]
        {\footnotesize ``Verify extracted figures match the original filing''}
      };
    \end{tikzpicture}
  \end{center}
\end{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      \foreach \task/\human/\ai/\y in {
        {Read one 10-K filing}/4 hours/2 min/4,
        {Compare 5 companies}/2 days/5 min/2.8,
        {Screen 50 filings for risk X}/1 week/15 min/1.6,
        {Quarterly portfolio review}/2 weeks/1 hour/0.4} {
        \node[font=\small, anchor=west, text width=4.2cm] at (0,\y) {\task};
        \fill[accent4!60, rounded corners=3pt] (4.5,\y-0.15) rectangle (7.8,\y+0.15);
        \node[font=\footnotesize, anchor=west, white] at (4.6,\y) {\human};
        \fill[accent2!70, rounded corners=3pt] (8.2,\y-0.15) rectangle (9.2,\y+0.15);
        \node[font=\footnotesize, anchor=west, white] at (8.3,\y) {\ai};
      }

      % Legend
      \fill[accent4!60, rounded corners=2pt] (4.5,-0.7) rectangle (5.1,-0.4);
      \node[font=\footnotesize, anchor=west] at (5.2,-0.55) {Human};
      \fill[accent2!70, rounded corners=2pt] (7,-0.7) rectangle (7.6,-0.4);
      \node[font=\footnotesize, anchor=west] at (7.7,-0.55) {AI Agent};
    \end{tikzpicture}
  \end{center}
...
\begin{frame}{Governance Framework}
  \begin{center}
    \begin{tikzpicture}[
      layer/.style={rounded corners=8pt, minimum width=10cm, minimum height=1.3cm,
      ...
      \node[layer, fill=accent3!10] (l2) at (0,1.9) {
        \textbf{Source Citations} --- Every claim traceable to a filing \& page
      };
      \node[layer, fill=accent!10] (l3) at (0,0.3) {
        \textbf{Audit Trail} --- Full log of reasoning, tool calls, and sources
      };
      \node[layer, fill=accent2!10] (l4) at (0,-1.3) {
        \textbf{Access Controls} --- Data classification, API keys, PII filtering
      };
